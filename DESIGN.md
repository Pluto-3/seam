...
- **Phase 0 — headless mechanic test. DONE, 2026-07-15.** No rendering, no leads, no player, no exporters. A small graph (tens of nodes), a small crowd (tens of agents) running move/gather/consume/trade/rest/craft/signal, fast-forwarded (not real-time — run as many ticks as compute allows) with summary stats printed every N ticks (population, average resources held, trade volume, most-used routes). Answer: **yes** — scarcity + specialization produces real, growing trade, confirmed by a negative control (specialization index 0.27 without trade vs. 0.42-0.54 with it, across seeds) and a hand-traceable small run. Population sustainability and unbounded food hoarding (both originally flagged as follow-ups) were root-caused and mostly fixed on 2026-07-15 — see `LOG.md` for the three real bugs found (stranded fractional food, unbounded idle gathering, and agents topologically blind to food beyond 1 hop) and the one legitimate-not-a-bug congestion dynamic left open. Signal→routing feedback remains disabled pending a non-flat scoring design.
- **Phase 1 — real-time rendering. BUILT, 2026-07-15, pending visual review.** A pygame live view (`watch.py`/`render.py`/`layout.py`) reusing the unmodified Phase 0 tick engine, with pause/speed controls and agents that glide between nodes rather than teleport. Runs cleanly (smoke-tested and confirmed stable on a real display), but whether it actually reads well is a human judgment call the build itself can't confirm — see `LOG.md`.
- **Phase 2 — leads + the hatch. DONE, 2026-07-15.** LLM-driven leads (Ollama, local, no API key needed) that pick from the crowd's own legal candidate list rather than inventing actions — verified goal-differentiated (a "wealthiest trader" lead trades almost every turn; a "keep wood stocked" lead shows a broader mix) with a proven argmax fallback if the model fails. Standing orders (leads → crowd) verified to shift crowd attention 6.13x at the ordered node. The hatch: possess/release a dedicated player character with a live keymap, wired into the same live view as the leads. Full story, including two real diagnostic misses caught before shipping, in `LOG.md`.
- **Phase 3 — the three exporters. DONE, 2026-07-15.** `export_data.py` (manifest + flattened CSV), `export_narrative.py` (lead/death events as readable markdown), `export_strategy.py` (single-run success analysis + a reusable before/after compare mode). All three read-only against an existing log, zero imports from the tick engine, verified. Strategy exporter's definition was reinterpreted from the original wording here — see `LOG.md` for why. Fahamu/LAUA/Waypoint integration still untouched, per the reuse principle: nothing about building these three needed it.
